...
rate_quantiles <- quantile(rlnorm(1000, meanlog = log(1e-5), sdlog = 0.1), c(0.5, 0.025, 0.975))
rate_quantiles
...
lines(c(-15, 115), rep(log10(rate_quantiles[1]), 2), lty = 2)
lines(c(-15, 115), rep(log10(rate_quantiles[2]), 2), lty = 3)
lines(c(-15, 115), rep(log10(rate_quantiles[3]), 2), lty = 3)
...
for(i in 1:length(unbalanced)){
  unbalanced_stats[i,] <- c(unbalanced[i], get_ci_clock_rate(unbalanced[i]))
}
