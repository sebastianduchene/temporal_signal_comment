...
setwd("~/Dropbox/projects_WORKING/temporal_signal_comment/simulation_scripts_remaster/")
...
for(i in 1:100){
  ...
  max_time <- c(max_time, diff(range(sampling_times)))
  
  # This is biased sampling because older dates have a lower probability of being sampled
  #sampled_data <- sort(sample(sampling_times, size = 100, replace = T, prob  =  (max(sampling_times) / sampling_times)))
  # This is uniform sampling
  sampled_data <- sort(sample(sampling_times, size = 100, replace = T)) 
  ...
  xml_input <- readLines("~/Dropbox/projects_WORKING/temporal_signal_comment/simulation_scripts_remaster/coalescent_heterochronous.xml")
  xml_input <- gsub("INPUT_TIMES", times_for_xml, xml_input)
  xml_input <- gsub("INPUT_NUM_SAMPLES", samples_for_xml, xml_input)
  writeLines(xml_input, con = "test_out.xml")
  
  system("~/phyloApps/beast2.7.6/bin/beast -overwrite test_out.xml")
